	...
	factory: (require) => {
		...
		const STRINGS = {
			tr: {
				...
				"f.cliModel.hint": "Görev bu CLI'da seçtiğin modelle koşar (iletim CLI'ye göredir: --model, --agent veya model-adı eki). Liste CLI'dan canlı çekilir.",
				"f.cliEffort": "Effort",
				"f.cliEffort.blank": "CLI varsayılanı",
				"f.cliEffort.hint": "Akıl yürütme eforu — agy --effort; vibe'da effort bayrağı yok (düşünme model girdisinde); devin'de modele eklenir (opus + high → opus-high).",
			...
			en: {
				...
				"f.cliModel.hint": "The task runs on this CLI with the model you pick (passed the way the CLI expects: --model, --agent, or a model-name suffix). The list is live from the CLI.",
				"f.cliEffort": "Effort",
				"f.cliEffort.blank": "CLI default",
				"f.cliEffort.hint": "Reasoning effort — agy --effort; vibe has no effort flag (thinking is baked into the model entry); for devin it composes into the model id (opus + high → opus-high).",
		...
		const css = [
			...
			".sa-act{border:1px solid var(--dsw-alias-border-l1);border-radius:10px;background:var(--dsw-alias-bg-layer-2);padding:10px 12px;display:flex;flex-direction:column;gap:6px}",
			".sa-act-title{font-size:10.5px;font-weight:800;letter-spacing:.8px;text-transform:uppercase;color:var(--sa);display:flex;align-items:center;gap:7px}",
			".sa-act-dot{width:7px;height:7px;border-radius:50%;background:var(--sa);animation:sa-pulse 1.4s ease-in-out infinite}",
			"@keyframes sa-pulse{50%{opacity:.35}}",
			".sa-act-row{display:flex;align-items:center;gap:8px;font-size:12.5px;min-width:0}",
			".sa-act-row b{font-weight:700;flex:none}",
			".sa-act-task{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--dsw-alias-label-secondary)}",
			".sa-act-ms{font:500 10.5px ui-monospace,monospace;color:var(--dsw-alias-label-tertiary);flex:none}",
			".sa-act-prev{font-size:11.5px;color:var(--dsw-alias-label-tertiary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}",
			".sa-act-done{color:#2b6a3f}.sa-act-failed{color:#c92a2a}",
			".sa-mini{position:fixed;right:18px;bottom:18px;z-index:850;display:flex;align-items:center;gap:8px;background:#18181d;border:1px solid #d9480f;color:#f0f0f4;font:600 12.5px system-ui,sans-serif;border-radius:999px;padding:8px 14px;cursor:pointer;box-shadow:0 8px 28px rgba(0,0,0,.45);animation:sa-in .18s ease}",
			".sa-mini:hover{background:#202027}",
			".sa-mini .dot{width:8px;height:8px;border-radius:50%;background:#d9480f;animation:sa-pulse 1.4s ease-in-out infinite}",
			"@media (prefers-reduced-motion: reduce){.sa-panel,.sa-mini{animation:none}.sa-act-dot,.sa-mini .dot{animation:none}}",
		...
		function Editor({ t, data, catalog, original, onDone, onSaved }) {
			...
			const [cli, setCli] = react.useState(original && routeKindOf(original.route) === "cli" ? String(original.route).replace(/^cli:\s*/, "") : "agy");
			...
			const preview = [
				...
				...(kind === "cli" ? [`cli: ${cli}`, ...(cliModel.trim() !== "" ? [`cliModel: ${cliModel.trim()}`] : []), ...(cliEffort !== "" ? [`cliEffort: ${cliEffort}`] : [])] : []),
				`color: "${color}"`,
				...
				h("div", { className: "sa-field" },
					...
					kind === "cli" ? h("div", { style: { marginTop: 8 } },
						h("select", { className: "sa-select sa-cli", value: cli, onChange: (e) => { setCli(e.target.value); setCliModel(""); setCliEffort(""); } },
							...(catalog?.clis ?? ["agy", "vibe", "devin"]).map((c) => h("option", { value: c, key: c }, c))),
						h("div", { style: { marginTop: 8 } },
							...
							cliModels.length === 0
								...
								: h("select", { className: "sa-select sa-cli-model", value: cliModel, onChange: (e) => setCliModel(e.target.value) },
									...
									...cliModels.map((m) => h("option", { value: m.id, key: m.id }, m.note ? m.id + " — " + m.note : m.id)))),
						cliEfforts.length > 0 ? h("div", { style: { marginTop: 8 } },
						...
						h("div", { className: "sa-hint" }, t("f.cliModel.hint"))) : null,
				),
				h("div", { className: "sa-field" },
		...
		// ---------------------------------------------------------- manager ----
		/** Light 2s poll of /activity while the panel is open. */
		function useActivity(active) {
			const [act, setAct] = react.useState({ running: [], recent: [] });
			react.useEffect(() => {
				if (!active) return;
				let dead = false;
				const tick = async () => {
					try {
						const d = await getJSON(OWN + "/activity");
						if (!dead) setAct({ running: d.running ?? [], recent: d.recent ?? [] });
					} catch {}
				};
				tick();
		...
		function Activity({ t, active }) {
			const act = useActivity(active);
			if (!act.running.length && !act.recent.length) return null;
			return h("div", { className: "sa-act" },
				act.running.length ? h("div", { className: "sa-act-title" }, h("span", { className: "sa-act-dot" }), "Running (" + act.running.length + ")") : null,
				...act.running.map((r) => h("div", { className: "sa-act-row", key: r.id },
					h("b", null, r.slug),
					h("span", { className: "sa-act-task" }, r.task),
					h("span", { className: "sa-act-ms" }, fmtMs(Date.now() - r.startedAt)))),
				act.recent.length ? h("div", { className: "sa-act-title", style: { marginTop: act.running.length ? 4 : 0, color: "var(--dsw-alias-label-secondary)" } }, "Recent") : null,
				...act.recent.slice(0, 5).map((r) => h("div", { className: "sa-act-row", key: r.id },
					h("b", { className: r.phase === "done" ? "sa-act-done" : "sa-act-failed" }, r.phase === "done" ? "\u2713" : "\u2717"),
					h("span", { className: "sa-act-task", title: r.preview || "" }, r.slug + " \u2014 " + (r.preview || r.task)),
					h("span", { className: "sa-act-ms" }, fmtMs(r.ms)))),
			);
		}

		function Manager({ t, data, catalog, reload, savedFlash, editing, setEditing, active }) {
					...
					h("div", { className: "sa-shell" + (editing !== null ? " editing" : "") },
						state.error && state.data === null
							...
							: state.loading && !state.data
								...
								: h(Manager, { t, data: state.data ?? { agents: [], disabledAgents: [], diagnostics: [] }, catalog: state.catalog, reload: () => void reload(), savedFlash: () => { setFlash(true); void reload(); }, editing, setEditing, active: open }),
					),
				),
			);
		}

		/** Corner mini indicator: visible only while subagents are running. */
		function MiniIndicator() {
			...
			if (!n) return null;
			return h("div", { className: "sa-mini", onClick: () => window.dispatchEvent(new CustomEvent("subagents:toggle")) },
				h("span", { className: "dot" }),
				n + " subagent" + (n === 1 ? "" : "s") + " running",
			);
		}
		...
		function apply(ctx) {
			...
			ctx.slots.inject("shell.overlay", () => ctx.slots.register({
				name: "shell.overlay",
				id: "dsh-subagents-indicator",
				order: 80,
				label: "Subagents running indicator",
			}, MiniIndicator));
		}
