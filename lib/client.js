	...
	factory: (require) => {
		...
		const STRINGS = {
			tr: {
				...
				"f.cli": "CLI",
				"f.cliModel.ph": "CLI modeli (örn. minimax/minimax-m3-free) — boş = CLI varsayilanı",
				"f.cliModel.hint": "Görev bu CLIda seçtiğin modelle koşar (--model bayrağıyla).",
			...
			en: {
				...
				"f.cliModel.hint": "The task runs on this CLI with the model you pick (via its --model flag).",
		...
		const css = [
			...
			".sa-badge{font-size:9.5px;font-weight:750;letter-spacing:.4px;text-transform:uppercase;border-radius:99px;padding:2px 7px;flex:none;max-width:170px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
			".sa-b-model{background:var(--sa-soft);color:var(--sa)}",
			".sa-b-cli{background:color-mix(in srgb,#b8860b 14%,transparent);color:var(--sa-warn)}",
		...
		function RoleRow({ role, t, selected, onEdit, onToggle, onDelete, onCopy }) {
			...
			const badgeText = off ? t("disabled.tag") : (kind === "inherit" ? t("route.inherit") : String(role.route).replace(/^cli:\s*/, ""));
		...
		function Editor({ t, data, catalog, original, onDone, onSaved }) {
			...
			const preview = [
				...
				...(kind === "cli" ? [`cli: ${cli}`, ...(cliModel.trim() !== "" && cli !== "dsh" ? [`cliModel: ${cliModel.trim()}`] : [])] : []),
				`color: "${color}"`,
			...
			return h("div", { className: "sa-editor" },
				...
				h("div", { className: "sa-field" },
					...
					kind === "cli" ? h("div", { style: { marginTop: 8 } },
						h("select", { className: "sa-select", value: cli, onChange: (e) => setCli(e.target.value) },
							...(catalog?.clis ?? ["cmdc", "pi", "agy", "claude", "dsh"]).map((c) => h("option", { value: c, key: c }, c))),
						cli !== "dsh" ? h("input", {
							className: "sa-input", style: { marginTop: 8 }, value: cliModel,
							placeholder: t("f.cliModel.ph"), onChange: (e) => setCliModel(e.target.value),
						}) : null,
