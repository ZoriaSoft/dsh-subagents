	...
	factory: (require) => {
		...
		const STRINGS = {
			tr: {
				...
				"f.cliEffort.hint": "Akıl yürütme eforu — cmdc/agy/claude --effort, pi --thinking; devin'de modele eklenir (opus + high → opus-high).",
			...
			en: {
				...
				"f.cliEffort.hint": "Reasoning effort — cmdc/agy/claude --effort, pi --thinking; for devin it composes into the model id (opus + high → opus-high).",
				...
				h("div", { className: "sa-field" },
					...
					kind === "cli" ? h("div", { style: { marginTop: 8 } },
						h("select", { className: "sa-select sa-cli", value: cli, onChange: (e) => { setCli(e.target.value); setCliModel(""); setCliEffort(""); } },
							...(catalog?.clis ?? ["cmdc", "pi", "agy", "claude", "dsh", "vibe", "devin"]).map((c) => h("option", { value: c, key: c }, c))),
