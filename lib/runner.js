...
export function buildCliArgv(cli, prompt, systemPrompt, cliModel, cliEffort) {
    ...
    // prompts — dangerous mode mirrors the local devin-bridge default.
    const base = { cmdc: ['cmdc', '--no-session'], pi: ['pi', '--no-session'], agy: ['agy', '--disable-slash-commands'], claude: ['claude'], dsh: ['dsh', '--profile', 'headless'], vibe: ['vibe', '--auto-approve', '--output', 'text'], devin: ['devin', '--permission-mode', 'dangerous', '--respect-workspace-trust', 'false'] }[cli];
    const argv = [...base];
