...
export const CLI_RUNNERS = {
    ...
    vibe: { promptFlag: 'dash-p', systemPrompt: 'embed', modelFlag: '--agent' },
};
...
export function buildCliArgv(cli, prompt, systemPrompt, cliModel, cliEffort) {
    ...
    const base = { cmdc: ['cmdc', '--no-session'], pi: ['pi', '--no-session'], agy: ['agy', '--disable-slash-commands'], claude: ['claude'], dsh: ['dsh', '--profile', 'headless'], vibe: ['vibe', '--auto-approve'] }[cli];
    const argv = [...base];
