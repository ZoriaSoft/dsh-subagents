 ...
 *                   model (dsh-subagents extension: cmdc | pi | agy | claude |
 *                   dsh | vibe | devin)
...
const KNOWN_KEYS = new Set([
    'name', 'description', 'model', 'cli', 'cliModel', 'cliEffort', 'background', 'tools', 'disallowedTools', 'color', 'skills',
]);
...
export function parseDefinition(file, text) {
    ...
    if (cliEffort !== '' && cli === '') {
        diagnostics.push(`${file}: "cliEffort" without "cli" — ignored`);
    }
    // CLIs whose effort composes into the model id (CLI_EFFORTS `applies:
    // 'model-suffix'`, devin) cannot carry an effort without a model, and a
    // model that already carries a level must not be suffixed again.
    if (cliEffort !== '' && cli !== '' && CLI_EFFORTS[cli]?.applies === 'model-suffix' && cliModel === '') {
        diagnostics.push(`${file}: cliEffort requires cliModel for cli "${cli}" (effort composes into the model id)`);
    }
    if (cliEffort !== '' && cli !== '' && CLI_EFFORTS[cli]?.applies === 'model-suffix' && hasEffortToken(cliModel) && !cliModel.endsWith('-' + cliEffort)) {
        diagnostics.push(`${file}: model "${cliModel}" already carries an effort level — drop cliEffort or pick a base model id`);
    }
    const backgroundRaw = attrs['background'];
    const background = typeof backgroundRaw === 'string'
        ? (backgroundRaw.trim().toLowerCase() === 'true' ? true : backgroundRaw.trim().toLowerCase() === 'false' ? false : undefined)
        : undefined;
    ...
    const def = {
        ...
        cliEffort: cliEffort !== '' && cli !== '' ? cliEffort : undefined,
        background,
        tools: list('tools'),
...
export function definitionSignature(def) {
    ...
    return JSON.stringify([
        def.name, def.description, def.model, def.cli, def.cliModel, def.cliEffort, def.background,
...
export function serializeDefinition(def) {
    ...
    if (def.cli !== undefined) {
        ...
        if (def.cliEffort !== undefined)
            lines.push(`cliEffort: ${def.cliEffort}`);
    }
    if (def.background !== undefined)
        lines.push(`background: ${def.background}`);
