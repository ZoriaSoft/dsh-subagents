...
import { loadDefinitions, parseDefinition, serializeDefinition } from './definitions.js';
import { reconcile } from './reconcile.js';
import { loadSkillBodies, SKILL_NAME_RE } from './skills.js';
import { CLI_RUNNERS, messageText, resolveRunMode, runCli, runModelBackground, runModelForeground, Semaphore } from './runner.js';
import { cliCatalog, CLI_EFFORTS, hasEffortToken } from './climodels.js';
...
/** Role skills bundled with the plugin (precedence: user dir wins over these). */
...
function resolveConfig(config) {
    ...
    return {
        agentsDir: typeof config?.agentsDir === 'string' && config.agentsDir !== ''
            ? config.agentsDir
            : join(dshHome, 'agents'),
        skillsDirs: Array.isArray(config?.skillsDirs) && config.skillsDirs.length > 0
            ? config.skillsDirs.map(String)
            : [join(dshHome, 'subagent-skills'), BUNDLED_SKILLS_DIR],
        ...
        cliCwd: typeof config?.cliCwd === 'string' ? config.cliCwd.trim() : '',
        trustedHosts: Array.isArray(config?.trustedHosts) ? config.trustedHosts.map(String) : undefined,
    };
}
...
/** Machine route value: `provider/model`, `cli: <cli>` or '' (inherit). */
function routeValue(def) {
    if (def.cli !== undefined)
        return `cli: ${def.cli}`;
    return def.model ?? '';
}

/** Human-readable route label for listings. */
function routeLabel(def) {
    if (def.cli !== undefined) {
        const parts = [def.cli];
        if (def.cliModel !== undefined)
            parts.push(def.cliModel);
        if (def.cliEffort !== undefined)
            parts.push(def.cliEffort);
...
export function apply(ctx, config) {
    ...
    resolved.logger = ctx.logger;
    /** slug → definition */ const bySlug = new Map();
    /** tool name → disposer */ const disposers = new Map();
    /** run id → activity record (running + son 20 biten; panel /activity) */
    const runs = new Map();
    /** Keep at most 20 finished records; running ones are never trimmed. */
    function trimRuns(map) {
        const finished = [...map.entries()].filter(([, r]) => r.phase !== 'running');
        if (finished.length <= 20)
            return;
        for (const [id] of finished.slice(0, finished.length - 20))
            map.delete(id);
    }
    /** Close one run record with phase + result preview. */
    function finishRun(map, id, phase, text) {
        const rec = map.get(id);
        if (rec === undefined)
            return;
        rec.phase = phase;
        rec.ms = Date.now() - rec.startedAt;
        rec.preview = String(text ?? '').slice(0, 200);
    }
    let diagnostics = [];
    let syncing = false;
    ...
    /**
     * Resolve a definition's role skills (spawn-time, live reads) and surface
     * missing names as a warning. Never fatal: the role still runs, the
     * primary agent sees the gap in the tool result.
     * @param {any} def
     * @returns {Promise<{ name: string, content: string }[]>}
     */
    async function skillsFor(def) {
        if (def.skills === undefined || def.skills.length === 0)
            return [];
        const { bodies, missing } = await loadSkillBodies(resolved.skillsDirs, def.skills);
        if (missing.length > 0)
            ctx.logger?.warn?.(`dsh-subagents: ${def.name}: skill(s) not found, spawned without them: ${missing.join(', ')}`);
        return bodies;
    }
    ...
    function register(def) {
        ...
        const dispose = ctx.tools.register(defineTool({
            ...
            parameters: {
                ...
                run_in_background: {
                    ...
                    description: def.cli !== undefined
                        ...
                        : 'Model-backed roles run in the background by default (the result arrives later as a notice). Pass false to wait for the result in this call.',
                },
            },
            output: {
                schema: {
            ...
            async execute(args, exec) {
                ...
                const skillBodies = await skillsFor(def);
                const mode = resolveRunMode(def, args);
                const runId = mode === 'background' ? `bg-${Date.now()}-${Math.random().toString(36).slice(2, 8)}` : `fg-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
                const rec = {
                    id: runId,
                    slug: def.slug,
                    tool: toolName(def),
                    kind: def.cli !== undefined ? 'cli' : mode === 'background' ? 'bg' : 'fg',
                ...
                try {
                    if (def.cli !== undefined) {
                        const text = await runCli(def, prompt, resolved, skillBodies);
                        finishRun(runs, runId, 'done', text);
                        return { text };
                    }
                    if (mode === 'background') {
                        const { childId } = await runModelBackground(ctx, def, prompt, exec, resolved, skillBodies);
                        // Re-key the activity record to the durable childId —
                        // the subagent/end event reports by child id.
                        const rec = runs.get(runId);
                        rec.id = childId;
                        runs.delete(runId);
                        runs.set(childId, rec);
                        return { text: `Subagent "${def.name}" launched in the background. Its result arrives as a notice when it settles. Live status: Subagents panel.` };
                    }
                    const out = await runModelForeground(ctx, def, prompt, exec, resolved, skillBodies);
                    finishRun(runs, runId, 'done', out.text);
                    return { text: out.text };
                }
                catch (error) {
                    finishRun(runs, runId, 'failed', String(error?.message ?? error));
                    throw error;
                }
            },
        }));
    ...
    // Background child settled → close its activity record (the runtime
    // notice already reaches the calling agent; this is panel bookkeeping).
    ctx.on('subagent/end', (info) => {
        const rec = runs.get(String(info?.id ?? ''));
        if (rec === undefined || rec.phase !== 'running')
            return;
        rec.phase = info?.stopReason === 'completed' ? 'done' : 'failed';
        rec.ms = Date.now() - rec.startedAt;
        rec.preview = messageText(info?.lastAssistantMessage).slice(0, 200);
    });
    ...
    const tryMount = () => {
        ...
        if (webServer === undefined)
            return;
        mounted = true;
        // Plugin-route fence: the host webserver dispatches registered handlers
        // directly (verified dsh 0.1.5-rc.1 `dsh-host-webserver`) — the Host /
        // Origin browser-trust fence + browser auth live only on RPC channels
        // registered through client-connection, NOT here. These routes write
        // role files and leak role bodies, so the plugin carries its own
        // Host/Origin fence: loopback authorities always pass, extra
        // authorities come from config `trustedHosts` or `DSH_TRUSTED_HOSTS`
        // (comma-separated); a browser Origin must match the Host authority.
        const registerRoute = (route) => {
            webServer.register({ ...route, handler: fenced(resolved, route.handler) });
        };
        registerRoute({
            kind: 'exact',
            path: `${ROUTE_BASE}/activity`,
            handler: async (_req, res) => {
                const all = [...runs.values()];
                const running = all.filter((r) => r.phase === 'running');
                const recent = all.filter((r) => r.phase !== 'running').slice(-20).reverse();
                json(res, 200, {
                    running,
                    recent,
                    queue: {
                        active: resolved.semaphore?.active ?? 0,
                        waiting: resolved.semaphore?.waiting?.length ?? 0,
                    },
                });
            },
        });
        registerRoute({
            ...
            handler: async (_req, res) => {
                try {
                    ...
                    const cliCatalogData = await cliCatalog();
                    json(res, 200, { providers: grouped, clis: Object.keys(CLI_RUNNERS), cliCatalog: cliCatalogData });
                }
                catch (error) {
        ...
        registerRoute({
            ...
            handler: async (_req, res) => {
                ...
                let visible = [];
                try {
                    // knownNames, not visible: the panel must offer every
                    // registered tool (bash, read, write, …), not only the
                    // scoped-visible subset of the calling context.
                    visible = [...ctx.tools.view(undefined).knownNames];
                }
                catch { /* registry unavailable */ }
                const body = {
                    agentsDir: resolved.agentsDir,
                    skillsDirs: resolved.skillsDirs,
                    agents: [...bySlug.values()].map((def) => ({
                        name: def.name,
                        tool: toolName(def),
                        route: routeValue(def),
                        cliModel: def.cliModel,
                        cliEffort: def.cliEffort,
                        background: def.background,
                        routeLabel: routeLabel(def),
                    ...
                    disabledAgents: [],
                    availableTools: visible.filter((n) => !n.startsWith('agent_')),
                ...
                loadDefinitions(resolved.agentsDir, { includeDisabled: true })
                    .then(({ agents }) => {
                        body.disabledAgents = agents.filter((d) => d.disabled === true).map((d) => ({
                            ...
                            route: routeValue(d),
                            cliModel: d.cliModel,
                            cliEffort: d.cliEffort,
                            background: d.background,
                            routeLabel: routeLabel(d),
        ...
        registerRoute({
            ...
            handler: async (req, res) => {
                ...
                const cliModel = String(payload?.cliModel ?? '').trim();
                const cliEffort = String(payload?.cliEffort ?? '').trim();
                ...
                if (cliModel !== '' && cli === '')
                    return void json(res, 400, { error: 'cliModel requires cli' });
                if (cliModel !== '' && CLI_RUNNERS[cli]?.modelFlag === null)
                    return void json(res, 400, { error: `cli "${cli}" does not support a model flag` });
                if (cliEffort !== '' && cli === '')
                    return void json(res, 400, { error: 'cliEffort requires cli' });
                const effortLevels = CLI_EFFORTS[cli]?.levels ?? [];
                if (cliEffort !== '' && !effortLevels.includes(cliEffort))
                    return void json(res, 400, { error: `cli "${cli}" effort must be one of: ${effortLevels.join(', ') || '(none)'}` });
                // Effort that composes into the model id (CLI_EFFORTS
                // `applies: 'model-suffix'`) needs a base model id, and a
                // model that already carries a level must not be suffixed
                // twice.
                if (cliEffort !== '' && CLI_EFFORTS[cli]?.applies === 'model-suffix' && cliModel === '')
                    return void json(res, 400, { error: 'cliEffort requires cliModel for this cli (effort composes into the model id)' });
                // Also model-suffix only: flag-effort CLIs legitimately pair a
                // leveled model id with a separate effort flag (runtime
                // self-heal covers CLI rejections there).
                if (cliEffort !== '' && CLI_EFFORTS[cli]?.applies === 'model-suffix' && hasEffortToken(cliModel) && !cliModel.endsWith('-' + cliEffort))
                    return void json(res, 400, { error: `model "${cliModel}" already carries an effort level` });
                let background;
                if (payload?.background !== undefined) {
                    if (typeof payload.background !== 'boolean')
                        return void json(res, 400, { error: 'background must be true or false' });
                    background = payload.background;
                }
                if (background !== undefined && cli !== '')
                    return void json(res, 400, { error: 'background only applies to model-backed roles (CLI roles are always foreground)' });
                const skills = Array.isArray(payload?.skills)
                    ? payload.skills.map((s) => String(s).trim()).filter((s) => s !== '')
                    : [];
                if (skills.some((s) => !SKILL_NAME_RE.test(s)))
                    return void json(res, 400, { error: 'invalid skill name (kebab-case, e.g. code-review-checklist)' });
                ...
                if (background === undefined && originalFile !== '' && /^_?[A-Za-z0-9][A-Za-z0-9 ._-]*\.md$/.test(originalFile)) {
                    try {
                        const { def: prev } = parseDefinition(originalFile, await readFile(join(resolved.agentsDir, originalFile), 'utf8'));
                        background = prev?.background;
                    }
                    catch { /* new role or unreadable file — nothing to preserve */ }
                }
                const def = {
                    name,
                    description,
                    ...
                    ...(cli !== '' ? { cli } : {}),
                    ...(cli !== '' && cliModel !== '' ? { cliModel } : {}),
                    ...(cli !== '' && cliEffort !== '' ? { cliEffort } : {}),
    ...
    void cliCatalog(); // warm the CLI model cache so the editor opens with lists
}

// ---------------------------------------------------------------- http ----

/** Authorities always trusted without configuration: loopback names. */
const LOOPBACK_NAMES = new Set(['localhost', '127.0.0.1', '::1', '[::1]']);

/**
 * Hostname of an authority header value (`Host` / `Origin`), lowercased.
 * Undefined when unparsable.
...
/**
 * Configured extra trusted authorities: config `trustedHosts` (string array),
 * else `DSH_TRUSTED_HOSTS` (comma-separated), else none.
 * @param {any} resolved @returns {Set<string>}
 */
function trustedHosts(resolved) {
    const raw = Array.isArray(resolved.trustedHosts)
        ? resolved.trustedHosts
        : String(process.env.DSH_TRUSTED_HOSTS ?? '').split(',');
    return new Set(raw.map((h) => authorityHost(String(h).trim())).filter(Boolean));
}

/**
 * Wrap a route handler with the plugin-side Host/Origin fence (see
 * `registerRoute` in apply): reject requests whose Host is not a trusted
 * authority, and browser `Origin` headers that do not match the Host —
 * the same confused-deputy pair the host fence defends for RPC channels.
 * @param {any} resolved @param {(req: any, res: any) => Promise<void>} handler
 */
function fenced(resolved, handler) {
    const trusted = trustedHosts(resolved);
    return async (req, res) => {
        const host = authorityHost(String(req.headers?.host ?? ''));
        if (host === undefined || (!LOOPBACK_NAMES.has(host) && !trusted.has(host))) {
            json(res, 403, { error: 'untrusted host' });
            return;
        }
        const origin = String(req.headers?.origin ?? '');
        if (origin !== '') {
            const originHost = authorityHost(origin);
            if (originHost !== undefined && originHost !== host) {
