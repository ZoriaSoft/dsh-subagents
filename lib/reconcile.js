 ...
 * A definition object identity change (file edit → new object) re-registers
 * its tool so the closure captures the new definition.
 *
...
export function reconcile(current, fresh, isRegistered) {
    ...
    for (const [slug, def] of fresh) {
        const changed = current.get(slug) !== def;
